...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.byKey(
          const ValueKey<String>('onboarding-screen'),
        ),
        findsOneWidget,
      );
      ...
      expect(
        saved.lastDailyQuestionDate,
        '2026-07-16',
      );
      expect(
        saved.lastDailyQuestionId,
        answeredQuestion.id,
      );
      expect(
        saved.lastDailyCardId,
        answeredQuestion.relatedCardId,
      );
      expect(
        saved.ownedCardIds,
        contains(answeredQuestion.relatedCardId),
      );
    },
  );

  testWidgets(
    '既存利用者: 再起動後に翌日の問題へ回答できる',
    (tester) async {
      final store = InMemoryPreferenceStore();
      final repository = SaveRepository(store: store);
      final clock = MutableDateProvider(
      ...
      await repository.save(
        SaveData(onboardingCompleted: true),
      );
      ...
      expect(
        firstDaySave.lastDailyQuestionDate,
        '2026-07-16',
      );
      ...
      expect(
        secondDaySave.lastPlayedDate,
        '2026-07-17',
      );
      expect(
        secondDaySave.lastDailyQuestionDate,
        '2026-07-17',
      );
      expect(
        secondDaySave.lastDailyQuestionId,
        secondQuestion.id,
      );
      expect(
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.tap(
        find.byKey(
          const ValueKey<String>('settings-reset-data'),
        ),
      );
      await tester.pumpAndSettle();

      expect(
        find.text('すべてのデータを削除します。この操作は取り消せません。'),
        findsOneWidget,
      );
...
Future<void> _completeOnboarding(
  ...
  for (var index = 0; index < 2; index++) {
    await tester.tap(
      find.byKey(
        const ValueKey<String>('onboarding-next'),
      ),
    );
    await tester.pumpAndSettle();
  }
  ...
  await tester.tap(
    find.byKey(
      const ValueKey<String>('onboarding-start'),
    ),
  );
  await tester.pumpAndSettle();
...
Future<Question> _answerToday(
  ...
  final generated =
      dailyQuestionService.generateTodayQuestions(
    questions,
    allCards: cards,
    count: 1,
  );
